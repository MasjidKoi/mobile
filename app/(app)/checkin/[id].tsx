...
export default function CheckInResultScreen() {
  ...
  return (
    <SafeAreaView edges={["top", "bottom"]} className="flex-1 bg-background">
      <View className="flex-1 items-center justify-center gap-4 px-lg">
        <View
          className="h-20 w-20 items-center justify-center rounded-full"
          style={{ backgroundColor: success ? c["primary-soft"] : c["accent-gold-soft"] }}
        >
          <Feather name={icon} size={36} color={tone} />
        </View>
        ...
        {badges.length > 0 ? (
          <View className="w-full gap-2 rounded-md border border-border bg-surface p-4">
      ...
      <View className="gap-2 px-4 pb-2">
        {success ? (
          <Button
            ...
            onPress={() => router.replace({ pathname: "/review/[id]", params: { id: masjidId } })}
          />
        ) : tooFar ? (
